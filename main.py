...
app.layout = dbc.Container([
        ...
        dbc.Col([
            ...
            dbc.DropdownMenu(
                [
                    ...
                    dbc.Row(
                        [
                            dbc.Col(dbc.Button("Marcar Todos", id="select-all-origins", color="link", size="sm", className="p-1")),
                            dbc.Col(dbc.Button("Desmarcar Todos", id="deselect-all-origins", color="link", size="sm", className="p-1")),
                        ],
                        className="g-0 m-1",
                    ),
                    dbc.Checklist(
                    ...
                    dbc.Button("Aplicar Filtros", id="apply-origin-filter", color="primary", size="sm", className="m-1 w-100"),
                ],
                id='origin-dropdown',
    ...
    dcc.Store(id='applied-origin-filter', data=[]),
...
@app.callback(
    ...
    [Input('tabs', 'value'),
     ...
     Input('stored-periods', 'data'),
     Input('applied-origin-filter', 'data')]
)
def render_tabs_content(tab, city, period_start, period_end, view, data, periods, applied_origins): # Renderiza o conteúdo das abas
    try:
        if data is None:
            return dbc.Alert("Por favor, carregue um arquivo Excel para começar.", color="info")
        
        # Obter os dados com o novo filtro de origens
        df = get_current_data(data, city, period_start, period_end, applied_origins)
...
@app.callback(
    ...
    Output('applied-origin-filter', 'data'),
    [Input('stored-data', 'data'),
     Input('city-selector', 'value'),
     Input('period-start', 'value'),
     Input('period-end', 'value')],
...
def update_origin_checklist(data, city, period_start, period_end, current_selection):
    ...
    return options, origins, origins

# Callback para atualizar o label inicial
@app.callback(
    Output('origin-dropdown', 'label'),
    [Input('origin-checklist', 'value')],
    [State('origin-checklist', 'options')]
)
def update_dropdown_label(selected_values, options):
    ...
    if len(selected_labels) > 2:
        return f"{len(selected_labels)} origens selecionadas"
...
# Callback para os botões Marcar/Desmarcar Todos
@app.callback(
    Output('origin-checklist', 'value', allow_duplicate=True),
    [Input('select-all-origins', 'n_clicks'),
     Input('deselect-all-origins', 'n_clicks')],
    [State('origin-checklist', 'options')],
    prevent_initial_call=True
)
def update_all_selections(select_clicks, deselect_clicks, options):
    ctx = dash.callback_context
    if not ctx.triggered:
        return dash.no_update
    
    button_id = ctx.triggered[0]['prop_id'].split('.')[0]
    if button_id == 'select-all-origins':
        return [opt['value'] for opt in options]
    elif button_id == 'deselect-all-origins':
        return []
...
@app.callback(
    Output('applied-origin-filter', 'data', allow_duplicate=True),  # Linha modificada
    Input('apply-origin-filter', 'n_clicks'),
    State('origin-checklist', 'value'),
    State('origin-checklist', 'options'),
    prevent_initial_call=True
)
def apply_origin_filter(n_clicks, selected_values, options):
    if n_clicks is None:
        return dash.no_update
